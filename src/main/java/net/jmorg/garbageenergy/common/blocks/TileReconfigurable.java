...
public abstract class TileReconfigurable extends TileRSControl implements IReconfigurableFacing, IReconfigurableSides, ISidedTexture
{
    ...
    public void readFromNBT(NBTTagCompound nbt)
    {
        ...
        facing = !nbt.hasKey("Facing") ? 3 : nbt.getByte("Facing");
        sideCache = getSideCacheFromNBT(nbt, getDefaultSides());
        for (int i = 0; i < 6; i++) {
    ...
    protected static byte[] getSideCacheFromNBT(NBTTagCompound tag, byte[] defaultSides)
    {
        if (tag == null) {
